                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        <div class="text-center mb-8">
                            ...
                            <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">CRM'e hoş geldiniz</div>
                        ...
                        <div>
                            ...
                            <input pInputText id="email1" type="text" placeholder="demo@crm.local" class="w-full md:w-120 mb-8" [(ngModel)]="email" />

                            <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Şifre</label>
                            <p-password id="password1" [(ngModel)]="password" placeholder="demo0" [toggleMask]="true" styleClass="mb-4" [fluid]="true" [feedback]="false"></p-password>
                            ...
                            <div class="text-sm text-muted-color mb-4">Demo hesabı: <strong>demo@crm.local</strong> / <strong>demo0</strong></div>
                            @if (loginError) {
                                <small class="block text-red-500 mb-3">{{ loginError }}</small>
                            }
                            <p-button label="Giriş yap" icon="pi pi-sign-in" styleClass="w-full" (onClick)="login()"></p-button>
                        </div>
                    </div>
                </div>
            </div>
...
export class Login {
    email: string = 'demo@crm.local';

    password: string = 'demo0';
    ...
    login(): void {
        this.loginError = '';
        if (this.authService.login(this.email, this.password, this.checked)) {
            this.router.navigateByUrl('/');
            return;
        }
        this.loginError = 'E-posta veya şifre hatalı. Demo hesabı: demo@crm.local / demo0';
    }
}
